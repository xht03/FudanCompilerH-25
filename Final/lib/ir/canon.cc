...
void CanonVisitor::visit(FuncDecl* node)
{
    ...
    if (node->blocks != nullptr) {
        for (auto block : *node->blocks) {
            ...
            block->accept(*this);
            if (b_result != nullptr)
                bl->push_back(b_result);
        }
    }
    fd_result
        = new FuncDecl(node->name, node->args, bl, node->return_type, visitor_temp_map->next_temp - 1, visitor_temp_map->next_label - 1);
}
    ...
    if (node->sl != nullptr) {
        for (auto stm : *node->sl) {
            ...
            if (sl_result != nullptr)
                sl->insert(sl->end(), sl_result->begin(), sl_result->end());
        }
    }
    if (sl->size() == 0)
        sl = nullptr;
...
void CanonVisitor::visit(Cjump* node)
{
    ...
    pairList->push_back(left_visit_result);
    pairList->push_back(right_visit_result);
...
void CanonVisitor::visit(Move* node)
{
    ...
    cout << "dst_visit_result->second: "
         << ((dst_visit_result->second == nullptr) ? 0 : kindToString(dst_visit_result->second->getTreeKind())) << endl;
    cout << "src_visit_result->second: "
         << ((src_visit_result->second == nullptr) ? 0 : kindToString(src_visit_result->second->getTreeKind())) << endl;
#endif
...
void CanonVisitor::visit(Seq* node)
{
    ...
    for (auto stm : *sl) {
        ...
        stm->accept(*this);
        if (sl_result != nullptr)
            seq_result->insert(seq_result->end(), sl_result->begin(), sl_result->end());
    }
    if (seq_result->size() == 0)
        seq_result = nullptr;
...
void CanonVisitor::visit(Call* node)
{
    ...
    if (visit_result->second->getTreeKind() != Kind::TEMPEXP && visit_result->second->getTreeKind() != Kind::CONST) {
