...
{
    visit_results.push_back(new QuadLabel(node, node->label, new set<Temp*>(), new set<Temp*>()));
}
...
{
    visit_results.push_back(new QuadJump(node, node->label, new set<Temp*>(), new set<Temp*>()));
}
