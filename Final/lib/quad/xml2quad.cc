...
#endif
    if (string(typeStr) == "INT")
        return tree::Type::INT;
    if (string(typeStr) == "PTR")
...
static QuadProgram* parseProgram(XMLElement* element)
{
    ...
    for (XMLElement* funcElem = element->FirstChildElement("funcdecl"); funcElem; funcElem = funcElem->NextSiblingElement("funcdecl")) {
...
static QuadFuncDecl* parseFuncDecl(XMLElement* element)
{
    ...
    if (paramsElem) {
        for (XMLElement* paramElem = paramsElem->FirstChildElement("param"); paramElem;
    ...
    for (XMLElement* blockElem = element->FirstChildElement("block"); blockElem; blockElem = blockElem->NextSiblingElement("block")) {
...
static QuadBlock* parseBlock(XMLElement* element)
{
    ...
    if (exitsElem) {
        for (XMLElement* exitElem = exitsElem->FirstChildElement("exit"); exitElem; exitElem = exitElem->NextSiblingElement("exit")) {
    ...
    for (XMLElement* stmtElem = element->FirstChildElement(); stmtElem; stmtElem = stmtElem->NextSiblingElement()) {
...
static QuadStm* parseStatement(XMLElement* element)
{
    ...
    if (strcmp(kind, "MOVE") == 0) {
        ...
        if (argsElem) {
            for (XMLElement* argElem = argsElem->FirstChildElement("arg"); argElem; argElem = argElem->NextSiblingElement("arg")) {
...
static set<Temp*>* parseDefUse(XMLElement* element, const char* defOrUse)
{
    ...
    for (XMLElement* tempElem = setElem->FirstChildElement("temp"); tempElem; tempElem = tempElem->NextSiblingElement("temp")) {
