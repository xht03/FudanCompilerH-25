...
public:
  ...
  Block(Label *entry_label, std::vector<tree::Label *> *exit_labels,
...
public:
  ...
  std::vector<tree::Block *>
      *blocks; // 第一个块是入口块 (block[0]的第一个标签是入口标签)
  Type return_type;
  int last_temp_num;  // 函数中使用的最后一个临时变量编号
  int last_label_num; // 函数中使用的最后一个标签编号

  FuncDecl(string name, std::vector<tree::Temp *> *args,
           std::vector<tree::Block *> *blocks, Type return_type, int lt, int ll)
      : name(name), args(args), blocks(blocks), return_type(return_type),
        last_temp_num(lt), last_label_num(ll) {}
...
public:
  Temp *temp; // 临时变量
  vector<pair<tree::Temp *, tree::Label *>>
      *args; // Label 是该值定义所在基本块的标签
