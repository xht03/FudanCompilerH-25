...
void DataFlowInfo::computeLiveness() {
    ...
    cout << "}" << endl;
                ...
                // BUT: if the current statment is return, or exit, then there is no successor!
                if (nextStmtInBlock[stmt] == nullptr) { //no next statement in block
                    for (auto succStmt : blockSuccStatements[block]) {
#ifdef DEBUG
...
map<string, DataFlowInfo*>* dataFlowProg(QuadProgram* prog) {
    if (!prog || !prog->quadFuncDeclList) return nullptr;
    map<string, DataFlowInfo*>* allDataFlows = new map<string, DataFlowInfo*>();
...
#endif
        (*allDataFlows)[func->funcname]=dfInfo;
    }
    return allDataFlows;
}
